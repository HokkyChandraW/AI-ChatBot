...
sentences_tokens = nltk.sent_tokenize(
    lowerData)  # converts to list of sentences
...
greetings = ['hey', 'hello', 'hi', 'hey dude', 'halo beb']
bye = ['bye', 'bye-bye', 'goodbye', 'have a good day']
thank_you = ['thanks', 'thank you', 'terimakasih',
            'thank you very much', 'terima kasih', 'thank you so much']
thank_response = ['sama-sama', 'senang bisa membantu',
...
def response(user_response):
    ...
    req_tfidf = flat[-2]
    if(req_tfidf == 0):
        chatterBox_response = chatterBox_response + \
            "Maaf, kami tidak dapat memproses pertanyaan Anda."
...
def bot_initialize(user_msg):
    if(user_msg == '/start'):
        bot_resp = """Hallo! Nama saya adalah Comi dari Rumah Sakit 'Corona Killer', apakah ada yang bisa saya bantu? \n(Format Booking Kamar Rawat Inap)\n\n[Booking Tempat] \nNama :\nWaktu: (ex: Senin, 12-01-2021, 21.00) \n\nKlik Bye untuk keluar."""
        return bot_resp
    elif('[booking tempat]' in user_msg.lower()):
        return 'Reservasi berhasil!'
    elif(user_msg.lower() == 'Selamat Pagi'):
        return "Selamat Pagi"
